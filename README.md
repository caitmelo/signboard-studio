...
Source snapshot matching hosted version 46, source commit `9c3489f7a6e8593cc6dfdda06abee92dce598e87`.
...
`webdev-signboard-studio-ai/` contains the existing separate server-backed application. Its Upload your template flow supports browser-side PDF intake and field review; Create your own accepts PDFs, JPGs and PNGs for server-side recognition. Credentials stay server-side. Its live recognition test is opt-in with `RUN_LIVE_AI_TEST=true`. This directory is unchanged by the version 46 snapshot update.
